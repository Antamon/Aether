...
try {
    ...
    $input = aetherValidateInput(
        $requestData,
        aetherCharacterRequestSchema('getCharacterDiary', $requestData)
    );
...
try {
    ...
    aetherJsonResponse(aetherBuildCharacterDiaryReadModel($pdo, $input['idCharacter']));
} catch (Throwable $e) {
    error_log('getCharacterDiary.php failed: ' . $e->getMessage());
    aetherJsonError(500, 'Kon diary niet ophalen.');
}
