...
$input = aetherReadCharacterJsonRequest('saveCharacterSecuritiesPortfolio');
...
try {
    ...
    if ($dateEnd !== '') {
        try {
            $eventEnded = (new DateTimeImmutable($dateEnd)) < new DateTimeImmutable('today');
        } catch (Throwable $e) {
            $eventEnded = false;
        }
    }
    ...
    if ($amount < 0) {
        http_response_code(400);
        echo json_encode(['error' => 'Geef een geldig bedrag op om uit de effectenportefeuille op te nemen.']);
        exit;
    }

    $previousWithdrawalAmount = round((float) ($snapshot['securitiesSnapshotWithdrawalAmount'] ?? 0), 2);
    $previousWithdrawalBankAmount = round((float) ($snapshot['securitiesSnapshotWithdrawalBankAmount'] ?? 0), 2);
    if ($previousWithdrawalAmount > 0 && $previousWithdrawalBankAmount <= 0) {
        $previousWithdrawalBankAmount = $previousWithdrawalAmount;
    }

    $availableAmount = round((float) ($character['securitiesaccount'] ?? 0), 2) + $previousWithdrawalAmount;
    ...
    $bankAmount = calculateCharacterSecuritiesSnapshotWithdrawalBankAmount($amount, $eventEnded);
    $deltaSecuritiesAmount = round($amount - $previousWithdrawalAmount, 2);
    $deltaBankAmount = round($bankAmount - $previousWithdrawalBankAmount, 2);
    $transactionId = (int) ($snapshot['securitiesSnapshotWithdrawalTransactionId'] ?? 0);
    $transactionDate = trim((string) ($snapshot['transactionDate'] ?? ''));
    if ($transactionDate === '') {
        $transactionDate = getDefaultBankTransferDate();
    }
    $description = buildCharacterSecuritiesSnapshotWithdrawalDescription(
        $idSnapshot,
        trim((string) ($snapshot['eventTitle'] ?? ''))
    );
    ...
    if ($deltaSecuritiesAmount !== 0.0 || $deltaBankAmount !== 0.0) {
        $pdo->prepare(
            'UPDATE tblCharacter
                SET bankaccount = ROUND(COALESCE(bankaccount, 0) + :bankDeltaAmount, 2),
                    securitiesaccount = ROUND(COALESCE(securitiesaccount, 0) - :securitiesDeltaAmount, 2)
              WHERE id = :idCharacter'
        )->execute([
            'bankDeltaAmount' => $deltaBankAmount,
            'securitiesDeltaAmount' => $deltaSecuritiesAmount,
            'idCharacter' => $idCharacter,
        ]);
    }
    ...
    if ($amount <= 0) {
        ...
        $pdo->prepare(
            'UPDATE tblCharacterSecuritiesTransaction
                SET securitiesAmount = :securitiesAmount,
                    bankAmount = :bankAmount,
                    transactionDate = :transactionDate,
    ...
    echo json_encode([
        'success' => true,
        'bankAmount' => $bankAmount,
        'hasLoss' => $eventEnded || $amount > 30000,
    ]);
    exit;
} catch (Throwable $e) {
