...
function canEditCharacterSecuritiesAccount(array $character, string $role): bool
{
    return isPrivilegedUserRole($role)
        && (string) ($character['state'] ?? '') !== 'draft';
}
...
function getCharacterBankTransactions(PDO $pdo, int $idCharacter): array
{
    ...
    $snapshotCounterpartyName = getCharacterEconomyClass(['id' => $idCharacter]) === 'upper class'
        ? 'Dotatie'
        : 'Beroepsinkomsten';
    ...
    try {
        ...
        foreach ($stmtSnapshots->fetchAll(PDO::FETCH_ASSOC) as $row) {
            ...
            $transactions[] = [
                ...
                'counterpartyName' => $snapshotCounterpartyName,
                'amount' => $amount,
                'transactionDate' => (string) ($row['transactionDate'] ?? ''),
    ...
    try {
        $stmtSecuritiesSnapshots = $pdo->prepare(
            "SELECT
                ...
                COALESCE(withdrawalTx.bankAmount, 0) AS securitiesSnapshotWithdrawalBankAmount,
                e.title AS eventTitle
             FROM tblCharacterEconomySnapshot AS ces
             JOIN tblEvent AS e
               ON e.id = ces.idEvent
        ...
        foreach ($stmtSecuritiesSnapshots->fetchAll(PDO::FETCH_ASSOC) as $row) {
            ...
            if ($withdrawalAmount > 0) {
                $bankAmount = round((float) ($row['securitiesSnapshotWithdrawalBankAmount'] ?? 0), 2);
                if ($bankAmount <= 0) {
                    $bankAmount = $withdrawalAmount;
                }
...
function getCharacterEconomySnapshots(PDO $pdo, int $idCharacter, bool $includePendingSecurities = true): array
{
    ...
    $snapshots = array_map(static function (array $row) use ($today): array {
        ...
        return [
            ...
            'securitiesSnapshotWithdrawalBankAmount' => round((float) ($row['securitiesSnapshotWithdrawalBankAmount'] ?? 0), 2),
...
function buildCharacterSecuritiesSnapshotWithdrawalDescription(int $idSnapshot, string $eventTitle = ''): string
{
    $prefix = '[snapshot-withdrawal:' . max(0, $idSnapshot) . ']';
    $title = trim($eventTitle);
    if ($title === '') {
        return $prefix . ' Opname effectenportefeuille - Aether';
    }

    return $prefix . ' Opname effectenportefeuille - Aether - ' . $title;
}

function formatCharacterSecuritiesTransactionDescription(string $description): string
{
    if (preg_match('/^\[snapshot-withdrawal:\d+\]\s*/', $description) === 1) {
        return (string) preg_replace('/^\[snapshot-withdrawal:\d+\]\s*/', '', $description);
    }

    return $description;
}

function calculateCharacterSecuritiesSnapshotWithdrawalBankAmount(float $amount, bool $eventEnded): float
{
    if ($amount <= 0) {
        return 0.0;
    }

    if ($eventEnded || $amount > 30000) {
...
function getCharacterSecuritiesPortfolio(PDO $pdo, array $character): array
{
    ...
    $managerDisplayName = '';
    if ($managerType === 'bank') {
        $managerDisplayName = 'De bank';
    } elseif ($managerType === 'third' && $managerCharacterId > 0) {
        $managerDisplayName = getCharacterDisplayNameById($pdo, $managerCharacterId);
    } else {
        $managerDisplayName = formatCharacterDisplayName($character);
    }
...
function getCharacterSecuritiesTransactions(PDO $pdo, int $idCharacter): array
{
    ...
    $transactions = array_map(static function (array $row): ?array {
        ...
        return [
            ...
            'transactionDate' => (string) ($row['transactionDate'] ?? ''),
            'description' => formatCharacterSecuritiesTransactionDescription($description),
            'canDelete' => false,
        ];
    }, $rows);

    return array_values(array_filter($transactions, static fn (?array $transaction): bool => $transaction !== null));
}
